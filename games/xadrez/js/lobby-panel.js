...
async function inviteRoom() {
  ...
  const text = 'Te desafio para uma partida de Xadrez. Código: ' + roomCode + ' - ' + url.toString();

  try {
    if (navigator.share) {
      await navigator.share({ title: 'Desafio de Xadrez', text, url: url.toString() });
      return;
    }
...
initLobbyUI = function() {
  ...
  el('input-room', 'input', function(e) {
